...
import {SaveAdmin} from '../../Redux/Actions/AdminActions'
...
const Auth = () => {
  ...
  const handleSubmit = async(e:any) => {
    ...
    if(formData.ID!=='' && formData.Password!==''){
      setLoading(true);
   const response = await Dispatch(SaveAdmin({ID:formData.ID,Password:formData.Password}));
  ...
  return ( 
    ...
    <h1 className="text-white text-center font-bold text-lg">Admin Login</h1>
  ...
  <button onClick={()=>Navigate('/Login')} className="text-white ">Not a Admin? <span className="text-blue-500 hover:underline opacity-80  hover:opacity-100 transition-all ease-in-out">Login As Customer Here</span></button>
  </div>
